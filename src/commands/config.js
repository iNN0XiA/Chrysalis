...
module.exports = {
  ...
  run: async (client, message, command, args, lang, guildInfo) => {
    ...
    if (!requestedModule || validModules.indexOf(requestedModule) == -1) return message.channel.send({embeds:[{
...
async function switchModule(message, requestedModule, enable, guildInfo, lang) {
  ...
  if (targetModule.enabled == enable) return reply(message, {content:(enable ? lang.module_already_enabled : lang.module_already_disabled).replace('{0}', requestedModule)}, true); // Do not update database if nothing will change
  targetModule.enabled = enable;
  await update(requestedModule, message, guildInfo, lang, (enable ? lang.module_enabled : lang.module_disabled).replace('{0}', requestedModule));
